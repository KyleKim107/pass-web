...
public class StatisticsServiceTest {
    ...
    @Nested
    @DisplayName("통계 데이터를 기반으로 차트 만들기") // set name of testing.
    class MakeChartData {
    ...
    public void test_makeChartData() {
        ...
        //creat random data returned from query
        // statisticsService.makeChartData()에 있는 statisticsRepository.findByStatisticsAtBetweenAndGroupBy does not access to DB but just return statisticsList
        List<AggregatedStatistics> statisticsList = List.of(
                new AggregatedStatistics(to.minusDays(1), 15, 10, 5),
                new AggregatedStatistics(to, 10, 8, 2)
        );
